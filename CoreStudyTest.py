...
def adicionar_curso():
    ...
    sql = "INSERT INTO tbl_cursos (titulo_curso, descricao_curso, carga_hora_curso, fk_tbl_categoria_id_categoria) VALUES (%s, %s, %s, %s)"
...
def editar_modulo():
    ...
    id_modulo = input("Digite o ID do módulo: ")
...
def editar_aula():
    ...
    id_aula = input("Digite o ID da aula: ")
...
def buscar_por_usuario_curso():
    while True:
        ...
        busca = input("\nDigite o nome que deseja achar: ")
        parametro = (f"%{busca}%",)
        ...
        if opcao == "1":
            cursor.execute("SELECT id_usuario, nome_usuario, email_usuario FROM tbl_usuarios WHERE nome_usuario LIKE %s", parametro)
            usuarios = cursor.fetchall()
            ...
            cursor.execute("SELECT id_curso, titulo_curso FROM tbl_cursos WHERE titulo_curso LIKE %s", parametro)
            cursos = cursor.fetchall()
            print("\n[Cursos Encontrados]")
            if not cursos:
                print("Nenhum curso foi encontrado.")
            for c in cursos:
                print(f"  ID: {c[0]} | Título: {c[1]}")
...
def menu_aluno(nome_usuario, id_usuario):
...
def menu_admin():
    ...
    while True:
        ...
        print("7 - Buscar por usuários/cursos")
        print("0 - Sair")
    ...
    while True:
        ...
        if opcao == "1":
            ...
            if tipo_usuario == "ADMIN":
                ...
                print(f"\nBem-vindo, {nome_usuario}!")
                menu_aluno(nome_usuario, id_usuario)
            else:
                print("Acesso negado!")
        elif opcao == "0":

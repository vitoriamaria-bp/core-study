...
def adicionar_usuario():
    ...
    aceite = input("Aceita os termos de uso? (S/N): ").upper()
    if aceite != 'S': return
...
def excluir_conta(id_usuario):
    if input("Tem certeza que deseja excluir sua conta? (S/N): ").upper() == 'S':
        senha = getpass.getpass("Confirme sua senha: ")
        conexao = conectar()
        cursor = conexao.cursor()
...
def listar_cursos():
    ...
    for c in cursor.fetchall(): print(f"ID: {c[0]} | Título: {c[1]}")
    cursor.close()
    conexao.close()
...
def trilha_do_aluno():
    ...
    for m in cursor.fetchall(): print(f"[{m[0]}] {m[1]}")
    id_m = input("\nID do Módulo: ")
    print("\n--- AULAS ---")
    cursor.execute("SELECT titulo_aula, url_arqui_aula FROM tbl_aulas WHERE fk_tbl_modulos_id_modulo = %s", (id_m,))
    for a in cursor.fetchall(): print(f"▶ {a[0]} | {a[1]}")
    cursor.close()
    conexao.close()